...
import { PhonePasswordLoginInterface } from "../../../../utils/AuthHelper/AuthHelperInterface";
...
const HandlePhonePasswordLogin =
  ...
  async (context: HookContext): Promise<HookContext> => {
    ...
    const data = context.data as PhonePasswordLoginInterface;

    const credentialData: AuthCredential_GET | null =
      await AuthHelper.checkAuthCredentialPhone(data.phone, params);
    if (credentialData) {
      ...
      throw new BadRequest("Invalid Phone number ! ! !, No user found.");
    }
    context.data = data;
    return context;
  };

export default HandlePhonePasswordLogin;
